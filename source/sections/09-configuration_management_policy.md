...
Insight Rx standardizes and automates configuration management through the use of Chef/Salt scripts as well as documentation of all changes to production systems and networks. Chef and Salt automatically configure all Insight Rx systems according to established and tested policies, and are used as part of our Disaster Recovery plan and process.
...
2. No systems are deployed into Insight Rx environments without approval of the Insight Rx CTO.
3. All changes to production systems, network devices, and firewalls are approved by the Insight Rx CTO before they are implemented to assure they comply with business and security requirements.
...
6. Tooling to generate an up-to-date inventory of systems, including corresponding architecture diagrams for related products and services, is hosted on GitLab.
   ...
   * These scripts are used to generate the diagrams and asset lists required by the Risk Assessment phase of Insight Rx's Risk Management procedures ([§4.3.1](#4.3-risk-management-procedures)).
...
10. Insight Rx utilizes development and staging environments that mirror production to assure proper function.
11. Insight Rx also deploys environments locally using Vagrant to assure functionality before moving to staging or production.
12. All formal change requests require unique ID and authentication.
13. Insight Rx uses the [Security Technical Implementation Guides (STIGs)](http://iase.disa.mil/stigs/) published by the Defense Information Systems Agency as a baseline for hardening systems.
...
3. Critical infrastructure services such as logging, monitoring, LDAP servers, or Windows Domain Controllers must be configured with appropriate Salt states.
   * These Salt states have been approved by the VP Engineering and CTO to be in accordance with all Insight Rx policies, including setting appropriate:
...
1. Insight Rx uses automated tooling to ensure systems are up-to-date with the latest security patches.
